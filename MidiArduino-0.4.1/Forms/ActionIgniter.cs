...
namespace MidiArduino.Forms
{
    ...
    {
        public int Index;
        public ActionIgniter(int index)
        {
            InitializeComponent();
        ...
        public bool Condition(MidiEvent midiEvent)
        {
            ...
            else if (conditionalDrop.SelectedItem.Text == "Custom condition")
            {
                try
                {
                    //return ProcessExpression(conTxt.Text.Replace("{VALUE}", returnValue(midiEvent).ToString())).Result;
                    return ProcessExpression(MIDIButton.replaceAliases(this, conTxt.Text, midiEvent)).Result;
